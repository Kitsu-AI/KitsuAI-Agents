...
export class MetricsCache {
  ...
  /** Export all metrics as a plain object */
  ...
  /** Import metrics from a plain object */
  import(obj: Record<string, number>): void {
    const now = Date.now()
    for (const [k, v] of Object.entries(obj)) {
      this.cache.set(k, { key: k, value: v, updatedAt: now })
    }
  }
}
